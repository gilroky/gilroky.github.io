...
void Record::writeStaticServiceArray(void) {
	...
	int i = 11;
	int number = array[i];
	number == array; // Changed number == number to number == array to improve code quality - KG
}
...
void Record::writeDynamicServiceArray(void) {
	...
	number == array; // Corrected static memory - KG
	delete[] array;
}
     ...
     switch(ch2)
     {
       ...
       case '3':
         ...
         case '7': recordMenu.serviceMenu();
   		 getch();break; // Added missing break - KG
         case '8': break;
